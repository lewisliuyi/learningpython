...
assert len(ALLCOLORS)*len(ALLSHAPES)*2 >= BOARDHEIGHT * \
    BOARDWIDTH, 'Board is too big for the number of shapes/colors defined.'
...
def getRandomizedBoard():
    ...
    # caculate how many icons are needs
    numIconsUsed = int(BOARDWIDTH * BOARDHEIGHT / 2)
    icons = icons[:numIconsUsed] * 2  # make 2 of each
    random.shuffle(icons)

    # Creat the board data structure, with randomly placed icons.
    board = []
...
def getShapeAndColor(board, boxx, boxy):
    ...
    # color value for x,y
    #  spot is stored in board[x][y][1]
    return board[boxx][boxy][0], board[boxx][boxy][1]


def drawBoxCovers(board, boxes, coverage):
    # Draws boxes being covered/revealed."boxex" is a list
